...
@click.argument("name")
@click.pass_context
def pack_manifest(ctx, repository, outdir, quals, flavor, version, name):
    '''
    Produce a directory of product tarfiles and accompanying manifest
    ...
    outdir = Path(outdir)
    tardir = outdir / "tarfiles"
    prods = list()
    for seed in seeds:
        prods += ctx.obj.pack_manifest(seed, paths=repository, outdir=tardir)
    print (prods)               # fixme render to manifest and save
